...
class CarryContextStore(context: Context) {
    ...
    companion object {
        /*
         * Pre-rebrand name, kept on purpose — same reasoning as [TokenStore], where it is written
         * out in full. A SharedPreferences name identifies a file that already exists on every
         * installed device, and asking for a new name hands back an empty one instead of failing.
         * Renaming here would silently discard the carry-context of every designer mid-workshop,
         * which is exactly the state this class exists to preserve across process death.
         */

 ...
 * -------------------------------------------
 * The three endpoints belong in `data/WorkshopRepositoryApi.kt`, reached through `WorkshopRepository`.
 * They are here instead because `WorkshopRepository.kt` is being edited by another stream in this same
 * working tree right now, and adding a method to a file somebody else is rewriting is how one of the
 * two changes silently disappears. `api` is private to `WorkshopRepository`, so there is no way to

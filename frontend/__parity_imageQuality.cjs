...
exports.NAMED_VIEW_SLOTS = exports.NEAR_DUPLICATE_MAX_DISTANCE = exports.MIN_LONG_EDGE_PX = exports.BLUR_VARIANCE_FLOOR = exports.MIN_CONTRAST_STDDEV = exports.WORK_EDGE_PX = void 0;
exports.toGreyPlane = toGreyPlane;
exports.resampleGrey = resampleGrey;
exports.contrastStdDev = contrastStdDev;
exports.laplacianVariance = laplacianVariance;
exports.differenceHash = differenceHash;
exports.hammingDistance = hammingDistance;
exports.isBlurred = isBlurred;
exports.isUnderResolution = isUnderResolution;
exports.attachedImageFromMedia = attachedImageFromMedia;
exports.findQualityIssues = findQualityIssues;
exports.findMissingViews = findMissingViews;
exports.isMeasurableImage = isMeasurableImage;
exports.measureImageFile = measureImageFile;
/**
 ...
 */
exports.MIN_CONTRAST_STDDEV = 12;
/**
 ...
 */
exports.BLUR_VARIANCE_FLOOR = 60;
/**
 ...
 */
exports.NEAR_DUPLICATE_MAX_DISTANCE = 6;
/**
 ...
 */
function toGreyPlane(rgba, width, height) {
 ...
 */
function resampleGrey(plane, width, height) {
    const data = new Uint8ClampedArray(width * height);
 ...
 */
function laplacianVariance(plane) {
    ...
    for (let y = 1; y < height - 1; y += 1) {
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const response = data[index - width] + data[index + width] + data[index - 1] + data[index + 1] - 4 * data[index];
            sum += response;
 ...
 */
function isBlurred(measurement) {
    ...
    if (measurement.contrast < exports.MIN_CONTRAST_STDDEV)
        return false;
    return measurement.blurScore < exports.BLUR_VARIANCE_FLOOR;
}
function isUnderResolution(measurement) {
    return Math.max(measurement.width, measurement.height) < exports.MIN_LONG_EDGE_PX;
}
/** Lift the fields duplicate detection needs off a media row, tolerating the ones that predate it. */
function attachedImageFromMedia(media) {
 ...
 * and cannot refuse.
 */
function findQualityIssues({ measurement, checksum, attached }) {
    const findings = [];
    if (isBlurred(measurement)) {
        findings.push({
            flag: "BLUR",
            severity: "MEDIUM",
            message: `Sharpness score ${Math.round(measurement.blurScore)} — a sharp photograph normally scores ` +
                `well above ${exports.BLUR_VARIANCE_FLOOR} here. This one may be out of focus. If it is meant to be ` +
    ...
    if (isUnderResolution(measurement)) {
        findings.push({
            ...
            message: `${measurement.width}x${measurement.height} — a catalogue plate needs about ` +
                `${exports.MIN_LONG_EDGE_PX}px on the long edge to print sharply at A4. This will still be saved; it ` +
    ...
    if (identical.length) {
        findings.push({
            flag: "DUPLICATE",
            severity: "LOW",
            message: `The same file is already attached here as ${identical.map((item) => `"${item.label}"`).join(", ")} ` +
        ...
        const near = candidates
            .map((item) => ({
            item,
            distance: item.perceptualHash && measurement.perceptualHash
                ? hammingDistance(measurement.perceptualHash, item.perceptualHash)
                : Number.POSITIVE_INFINITY
        }))
            .filter((entry) => entry.distance <= exports.NEAR_DUPLICATE_MAX_DISTANCE)
            .sort((left, right) => left.distance - right.distance);
        if (near.length) {
            findings.push({
                flag: "DUPLICATE",
                severity: "LOW",
                message: `This looks like the same shot as ${near.map((entry) => `"${entry.item.label}"`).join(", ")}. ` +
 ...
 */
function findMissingViews(entityKey, filled) {
    const slots = exports.NAMED_VIEW_SLOTS[entityKey];
    if (!slots)
        return [];
    ...
    return [
        {
            flag: "MISSING_VIEW",
            severity: "LOW",
            message: `${present.length} of ${keys.length} views captured — no ${missing
...
function isMeasurableImage(file) {
 ...
 */
function readPixels(bitmap, width, height) {
...
async function measureImageFile(file) {
    if (typeof createImageBitmap === "undefined" || !isMeasurableImage(file))
        return null;
    ...
    try {
        ...
        if (width < 1 || height < 1)
            return null;
        const scale = Math.min(1, exports.WORK_EDGE_PX / Math.max(width, height));

...
import { API, apiToken, bearer, signIn } from "./support/session";
...
/**
 * THE SECTIONS HAVE TO EXIST, AND ON A FRESH DATABASE THEY DO NOT.
 *
 * Every assertion in this file is about a control that lives INSIDE a questionnaire section, and the
 * sections are content — `GET /questionnaire/sections` returns `[]` on a database nobody has seeded
 * one into. When that happened, all five tests sat for sixty seconds and then reported that a
 * `<details>` was missing, which reads as "the capture controls are broken" and means "there is
 * nothing to capture". Five red tests for a reason that is not in this repository is how a suite
 * stops being read.
 *
 * So it is checked once, up front, and the file skips saying exactly what is absent. This is
 * deliberately NOT solved by seeding a section: these tests assert the DEFAULTS a researcher meets
 * on the real corpus, and a section invented here would be asserting the fixture instead.
 */
...
test.beforeAll(async ({ request }) => {
  ...
  const res = await request.get(`${API}/api/questionnaire/sections`, { headers: bearer(token) });
  if (!res.ok()) return;
  const body = await res.json();
  sectionsExist = Array.isArray(body) && body.length > 0;
});

test.beforeEach(() => {
  test.skip(
    !sectionsExist,
    "this database has no questionnaire sections (GET /questionnaire/sections is empty), so there is no section for the capture controls to sit in"
  );
});

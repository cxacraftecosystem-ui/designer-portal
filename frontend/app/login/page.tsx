...
// The form itself, which `AppShell` renders too — the same component, so the two hosts cannot
// come to enforce different password rules. See its header.
...
const BRAND_POINTS = [
  ...
  // MINISTRY_ADMIN (48) landed. This line is the second copy of the sentence the landing hero
  // speaks — `HeroLanding.tsx:70` renders the same "<n>-tier access control" badge but DERIVES its
  // number from `AccessLadder.tsx`'s `TIER_COUNT_WORD`, i.e. from `ROLES_BY_RANK.length`, so the
  // hero re-counts itself on the day a tier ships and this page does not. It is left hand-written
  // rather than imported because `AccessLadder.tsx` is a framer-motion component and the login page
  // pulls in no animation library at all; the cost of importing one for a single word is worse than
  // the count. If you add a tier, this is one of the lines that will not tell you.
  //
  // AND THE WARNING ABOVE HAS NOW BEEN IGNORED TWICE, so read it as a defect report rather than as
  // advice. It shipped "Seven-tier" past INSPECTOR and was corrected by hand on 2026-08-27; it then
  // shipped "Eight-tier" past the three directorate tiers and was corrected by hand again on
  // 2026-09-14, after a review found it. Both times the hero badge one click away said the right
  // number, so a visitor could read two counts of the same ladder in one session — and for a
  // ministry-facing product the size of the access ladder is a compliance statement, not a slogan.
  // THE DURABLE FIX IS NOT A BETTER COMMENT: move `TIER_COUNT_WORD` out of `AccessLadder.tsx` into a
  // motion-free module (`lib/permissions.ts` already owns `ROLES_BY_RANK`, and `NUMBER_WORD` is a
  // plain lookup table with no framer import behind it) and let both pages read it. That costs this
  // page nothing and ends the genre.

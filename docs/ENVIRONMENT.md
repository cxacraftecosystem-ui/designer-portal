...
| `DATABASE_CONNECTION_LIMIT` | No | `10` | No | Client connections **per uvicorn worker**. Do not raise to 40: on the Supabase deployment this ran on until 2026-08-22 that tripped a 200-client pooler ceiling (`EMAXCONN`) and crash-looped startup. The *ceiling* was that provider's; the *lesson* — that this number multiplies by every worker, pod and rollout surge — is arithmetic and applies everywhere. **Whatever the current provider's ceiling is has not been established from this repository; see the open question in [KUBERNETES.md](KUBERNETES.md).** ⚠ **AND ON A DIRECT ENDPOINT, 10 IS AN INCREASE RATHER THAN A CAP — UNDECIDED.** `build_runtime_database_url` writes `connection_limit` when `DATABASE_USE_TRANSACTION_POOLER` is true **or** the limit was set explicitly, and that flag *defaults to true* while matching no hostname — it is an operator's declaration, not a fact about the DSN. So a direct DSN with the flag left alone is given `connection_limit=10`, and 10 was only ever evaluated as a REDUCTION against a pooler. Prisma would otherwise size the pool from the CPU count: on a 2-vCPU box that is 5, so the declaration doubles it. Nobody has been hurt by this and nobody has measured it. **Settle it in the deployment, not in code:** either point `DATABASE_URL` at the pooled endpoint (making the default true and correct) or set `DATABASE_USE_TRANSACTION_POOLER=false`, which also stops `pgbouncer=true` being added to a connection that does not want it. Recorded 2026-08-31 as finding **A30-11** in [AUDIT-2026-08-30.md](AUDIT-2026-08-30.md). |

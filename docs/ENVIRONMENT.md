...
**Production runs on Supabase.** *Recorded 2026-09-02.*

| | |
|---|---|
| Authority | `backend/.env.production` — it names a `supabase.co` host and no other database provider's. That file is the only thing that decides this, and it is gitignored, so **this table is a report of it, never a second source of truth.** |
| How to re-check without reading a secret | `grep -c supabase.co backend/.env.production` → a line count, never a line. That is the whole method: **presence of a host substring**, which is all the evidence this fact needs and the only evidence that can be gathered without handling a credential. Do not open the file, do not paste from it, do not connect with it. (The substring also matches the provider's `*.pooler.supabase.com` pooler hosts, which is what a pooled DSN names.) |
...
loopback or private? — `_is_local_db_host`) or on an explicit setting. That rule survives the
2026-09-02 move: the DSN names that same vendor's pooler again, and the code still treats it as an
opaque remote host — the port, the pooling declaration and the limit are all written into the
deployment's environment, never inferred.
...
| `SUPABASE_KEEPALIVE_URL` / `SUPABASE_DATABASE_URL` | `keep-supabase-active.yml` → `scripts/keep-supabase-active.mjs` | `SUPABASE_DATABASE_URL` is set; the other is an optional override | **Yes** | **Live again as of 2026-09-02** — dormant from 2026-08-22 until then, while the provider of that era suspended idle compute on its own. The provider hosting production today *pauses* a free-tier project after a stretch of inactivity, which is the exact case the nightly ping was built for, so the move restored the two `schedule:` lines and set the secret (the session DSN; the script rewrites the port itself). See [The database](#the-database-what-is-required-and-where-it-runs-today). |
| `SUPABASE_KEEPALIVE_NO_REWRITE` | same | No | No | `"true"` disables the script's `:5432 → :6543` rewrite. Leave it unset: the rewrite matches the `.pooler.supabase.com` host suffix, which is what the secret above names, and the ping belongs on the transaction port where it cannot occupy one of the session pool's 15 slots. **This is the script's own rewrite**; the equivalent in `backend/app/core/db.py` was removed on 2026-08-22 and is deliberately not coming back. |
| `SUPABASE_DB_SSL` | same | No | No | `"false"` disables TLS for the keep-alive connection; any other value keeps SSL on. Leave it unset. |

**The two secret inventories in this document disagreed, and the disagreement is settled.** An
undated `gh secret list` quoted in a workflow header used to be cited here as finding `BACKEND_ENV`,
`EC2_HOST` and `EC2_SSH_KEY` **and nothing else**, which would have meant `deploy-frontend.yml` was

...
**It is not a description of this repository, and that is the single most important thing to know
about it.** Every other document in `docs/` is checked against code in this checkout. This one is a
design-system extraction of an *external* product surface, kept here as a reference artefact for the
visual language — token naming, type-scale discipline, the "never document hover" rule — not as a
specification of what the Design Prototype Workshop web client renders. The workshop app's own
surface is `frontend/app/globals.css` and `frontend/tailwind.config.ts`; if the two ever disagree,
those files win and this document is not wrong, because it was never claiming to describe them.

That framing is what makes it maintainable at all. There is no command that verifies it, because the
thing it describes is a website nobody in this repository controls. What can be done:

| Claim | How to check |
|---|---|
| The colour, type and spacing tokens | Re-extract from the live surface. A token that no longer appears there is stale, not wrong-for-us |
| That nothing here is mistaken for a repository spec | Grep the repository for these token names. They should appear **nowhere** outside this file. If one has leaked into `frontend/`, either this document has been misread as a spec or the leak is deliberate and belongs documented in the frontend's own styles |

**Review triggers:** somebody proposing to adopt a token from here into `frontend/` — at which point
the value stops being a reference and becomes repository state, and belongs in the frontend's own
theme with its own provenance; a rewrite of the workshop client's visual language, which would make
a reader more likely to mistake this page for the spec and so needs the disclaimer above kept
prominent.

**Known unverified:** everything about the external surface, necessarily — colour values, type
scale, component inventory and the licensing status of the named typefaces were read off that
surface on the day this was written and are not re-checked by anything. Treat every hex value here
as a record of an observation, not as a current fact.

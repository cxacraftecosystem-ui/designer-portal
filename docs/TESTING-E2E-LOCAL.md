...
|---|---|
| The four commands, and their order — **executable by a machine, but only when asked** | `.github/workflows/e2e-live.yml`, added 2026-09-03. It is this document's "The sequence" section, scripted, in its order and for its stated reasons. **It ran every Monday until 2026-09-21, when the schedule was removed and it became `workflow_dispatch` only** — so it is no longer a machine asking this question on its own, and a person remembering to trigger it (`gh workflow run "Live end-to-end suite"`) is once again what keeps this section honest. That is a real weakening of this row and is recorded rather than glossed: the weekly runs were going red unread, which is why the cron went, and the workflow's header carries the evidence. Run it when you change the sequence below. If you change the sequence in one, change it in the other. **Two departures, both forced by CI and both documented in that file's own header:** the host ports are pinned to `55432` and `9010` in the job's `env:` (a runner has no repository-root `.env`, so the compose defaults apply and `55442` is an override on one laptop), and it runs `prisma migrate deploy`, which this document does not mention because the laptop's database already carried the schema. A runner's Postgres is empty every time. |

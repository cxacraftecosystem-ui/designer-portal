# AI features — background removal, layer separation, vectorisation, market research
...
**Three of the four capabilities are image operations, and the bulk of this document is about
them.** The fourth — **market research**, which goes and finds what comparable products cost and
who sells them — has its own document, [MARKET_RESEARCH.md](MARKET_RESEARCH.md), because it has a
different provider set, a different legal surface and a provenance problem the image features do
not have. Its variables are listed here in §3 with everything else, so this file stays the single
index of what the package reads. §10 summarises it.
...
| **Market research** | Comparable products: name, seller, price, currency, source URL, retrieval timestamp — see [MARKET_RESEARCH.md](MARKET_RESEARCH.md) | `AI_MARKET_RESEARCH_ENABLED` |
...
Seven providers. The three image capabilities have two each, one local and one hosted; market
research has three, described in [MARKET_RESEARCH.md §3](MARKET_RESEARCH.md). `auto` (the default)
...
| `local_catalogue` | local | market research | `AI_MARKET_RESEARCH_CATALOGUE_PATH` — **nothing to install** | none | 28 MB *(MEASURED)* | 0.18 s / 5k rows *(MEASURED)* | free |
| `serper_shopping` | hosted | market research | `SERPER_API_KEY` | none | ~20 MB *(ESTIMATED)* | 1–3 s/query *(ESTIMATED)* | 2,500 free *(VENDOR-STATED)*; ~$1.00→$0.30 per 1,000 *(THIRD-PARTY REPORTED)* |
| `serpapi_shopping` | hosted | market research | `SERPAPI_API_KEY` | none | ~20 MB *(ESTIMATED)* | 2–6 s/query *(ESTIMATED)* | 250/month free, $25/1,000 → $275/30,000 *(VENDOR-STATED)* |

Every number in those tables is labelled. **MEASURED** means it was run on a machine and the method
...
**THIRD-PARTY REPORTED** is a fourth label, added with the market-research providers: serper's
pricing page could not be read directly, so its per-credit figures came from third-party summaries
and are deliberately *not* labelled vendor-stated, because they are not. The same labels are
attached to the numbers in the code (`ResourceProfile.basis` and the `money` string) and are
printed by the probe, so nothing has to be looked up in a document to be trusted or distrusted
correctly.
...
| `AI_MARKET_RESEARCH_ENABLED` | `false` | Capability 4. Needs the master switch too. |

Two levels because the master switch is what an incident responder flips: one variable stops the
whole package without having to work out which of four features is misbehaving.
...
| `AI_MARKET_RESEARCH_PROVIDER` | `auto` | `auto`, `local_catalogue`, `serper_shopping`, `serpapi_shopping` |

`auto` prefers **hosted for matting** (the box cannot host U2Net), **local for vectorisation**
(VTracer has no model and no per-image charge) and **local for market research** (a curated price
list costs nothing per query, discloses nothing to anybody, and answers with no signal). An
explicit id is never silently substituted: if you ask for `rembg_local` and it is not installed,
you get an error saying so rather than a remove.bg charge you did not ask for.
...
Full treatment in [MARKET_RESEARCH.md](MARKET_RESEARCH.md); these are the exact names.

| Variable | Default | Meaning |
|---|---|---|
| `AI_MARKET_RESEARCH_MAX_RESULTS` | `20` | Listings per brief, **split across the planned queries** rather than given to each. Raising it costs money per call on a hosted provider. |
| `AI_MARKET_RESEARCH_MAX_QUERIES` | `3` | Queries per brief. Each one is a credit. A brief with no craft, category, materials or place plans exactly one whatever this says. |
| `AI_MARKET_RESEARCH_MIN_INTERVAL_SECONDS` | `1` | Courtesy delay between outbound calls, process-wide, per provider. `0` turns it off. Not rate-limit compliance — the vendor's quota is yours to respect. |
| `AI_MARKET_RESEARCH_CURRENCY` | `INR` | The currency the figures are reported in. Listings in another currency are **excluded and counted, never converted.** |
| `AI_MARKET_RESEARCH_COUNTRY` | `in` | The `gl` parameter both hosted providers take. |
| `AI_MARKET_RESEARCH_LANGUAGE` | `en` | The `hl` parameter both hosted providers take. |
| `AI_MARKET_RESEARCH_USER_AGENT` | identifies this software | Sent on every outbound request. **Override it to add a contact address**, so a vendor has somebody to reach before blocking a shared IP. |

### Market research — the offline provider

| Variable | Default | Meaning |
|---|---|---|
| `AI_MARKET_RESEARCH_CATALOGUE_PATH` | unset | Path to a `.csv`, `.tsv` or `.jsonl` price list with `name`, `seller`, `price`, `currency`, `url` and optionally `updated`. The only thing `local_catalogue` needs — there is nothing to install. |

### Market research — serper.dev

| Variable | Default | Meaning |
|---|---|---|
| `SERPER_API_KEY` | unset | **Secret.** The only thing needed to enable hosted market research. |
| `SERPER_ENDPOINT` | `https://google.serper.dev/shopping` | Override for a proxy or a test double. |

### Market research — SerpApi

| Variable | Default | Meaning |
|---|---|---|
| `SERPAPI_API_KEY` | unset | **Secret**, and it travels in the query string because that is the only place SerpApi accepts it — so a proxy that logs full URLs will log this key. |
| `SERPAPI_ENDPOINT` | `https://serpapi.com/search.json` | Override for a proxy or a test double. |
...
AI features: disabled (default)
  ...
  [off] market_research: AI_FEATURES_ENABLED is off - the whole package is dormant
        - local_catalogue: unset settings: AI_MARKET_RESEARCH_CATALOGUE_PATH
        - serper_shopping: unset settings: SERPER_API_KEY
        - serpapi_shopping: unset settings: SERPAPI_API_KEY
...
# market research has its own two files:
./.venv/Scripts/python.exe -m pytest tests/test_market_research.py tests/test_market_research_providers.py -q
```

55 tests for the image capabilities and 134 for market research, and they run **with none of the
optional dependencies installed** — which is the point.
...
- **No fields in `app/core/config.py`.** The package reads its own settings so that a strictly
  ...
  turned on for real, the 37 names in `ai_features/settings.py::ENV_VARS` can be moved into
...
- **No entry in `backend/.env.example`.** This document is the reference until somebody enables a
  capability; adding thirty-seven commented-out variables to the template would suggest they are
  expected.
...
## 10. Market research, in one page

The fourth capability, and the only one that is not an image operation.
**[MARKET_RESEARCH.md](MARKET_RESEARCH.md) is the document; this is the index entry.**

`research_market(brief)` takes a keyword or a `ResearchBrief` and returns comparable products —
name, seller, price, currency, source URL, retrieval timestamp — merged into the same
`PriceObservation` shape `app/services/market_analysis.py` already consumes, so the existing
distribution and price-band machinery runs on retrieved data unchanged.

Four things to know before reading further:

- **A retrieved price can never be counted as a respondent's.** It is stamped
  `source="RETRIEVED_LISTING"`, which is neither `RESPONDENT` nor `COMPETITOR`, so every existing
  filter in `market_analysis` excludes it with no new code; `assert_surveyed()` raises rather than
  filters if one gets into a surveyed sample. That is the property the whole design is arranged
  around.
- **Almost none of it is agentic.** Query planning, price parsing (including Indian digit grouping),
  currency normalisation, deduplication, outlier rejection and the merge are deterministic and
  unit-tested. The one thing that would genuinely need a model — reading a rambling free-text brief
  — is deliberately not built, because the app already captures craft, category, materials and
  district as controlled vocabularies. MARKET_RESEARCH.md §2 argues it properly.
- **Nothing crawls.** Every provider is a documented commerce API or a local file, which is why
  there is no robots.txt handling: the obligation was designed out rather than complied with.

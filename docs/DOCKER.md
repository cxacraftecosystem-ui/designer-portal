...
```bash
docker build -f backend/Dockerfile  -t design-workshop-backend:local  .
docker build -f frontend/Dockerfile -t design-workshop-frontend:local .
```
...
materialises for anyone who does not migrate — and `docker image rm design-workshop-migrate:local`

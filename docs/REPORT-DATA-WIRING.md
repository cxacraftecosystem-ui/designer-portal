...
| "Where the artisan lives is only on `ReferencedRecord`" | `REFERENCE_HYDRATION["participant.artisanRef"]` in `backend/app/services/stage_schema.py` — count its keys. It carries `state`, `district`, `pincode`, `address` and `subjectLocation` as well as `village`. **This row listed five stale code sites on 2026-08-19 and three of them had already been corrected when it was written; re-verified against the tree on 2026-08-20 and reduced to what is actually stale.** Corrected, and not to be "fixed" again: `ReferencedRecord` and `_artisan_points` in `report_builder.py` (both now quote the old claim and strike it), and the test formerly called `test_artisan_homes_come_from_the_referenced_artisan_record`, which **no longer exists under that name** — it is `test_a_roster_row_that_states_no_address_falls_back_to_the_referenced_artisan_record`, and the rename IS the correction, so a reader hunting the old symbol finds nothing and concludes the file was missed. Still stale, on 2026-08-20: `load_report_references` in `design_workshops.py`, whose second bullet still reads "No roster field holds a district — the participant row records a village as free text"; and `renderMap`'s header in `android/app/src/main/java/com/designprototype/workshop/ui/designworkshop/ReportFigures.kt` (**not** `report/ReportFigures.kt` — this row cited no path and the obvious guess is the wrong directory), which still says "the office resolves each roster row through the `Artisan` record it was picked from". Those two are stale, not authoritative; when one is corrected, this row is what it should be corrected TO. **Re-verify each site before naming it here** — the failure this row is repairing is naming a corrected file as stale, which sends the next reader to revert a fix. |

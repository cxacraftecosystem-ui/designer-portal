...
|  | dev | staging | prod |
| --- | --- | --- | --- |
| Namespace | `design-workshop-dev` | `design-workshop-staging` | `design-workshop-prod` |
...
| Secrets prefix | `design-workshop/dev/*` | `design-workshop/staging/*` | `design-workshop/prod/*` |
...
1. `DESIGN_WORKSHOP_ENV` (one string in the ConfigMap patch) is substituted into the environment segment
   ...
   `DESIGN_WORKSHOP_ENV` is not read by the application — `Settings` is `extra="ignore"`.
...
docker build -f backend/Dockerfile                 -t <registry>/design-workshop-api:<tag> .
# Migrations — the intermediate stage that still has Node.
docker build -f backend/Dockerfile --target prisma -t <registry>/design-workshop-migrate:<tag> .
```
...
`secretsmanager:GetSecretValue` on `arn:aws:secretsmanager:<region>:<account>:secret:design-workshop/<env>/*`
...
```bash
kubectl -n design-workshop-prod rollout restart deployment/design-workshop-api deployment/design-workshop-queue
```
...
```yaml
# overlays/dev/kustomization.yaml, in the design-workshop-api patch
readinessProbe:
  httpGet:
    path: /health/ready
    port: http
...
```bash
ENV=prod
NS=design-workshop-$ENV
REL=2026.07.26-1      # short: the Job name is design-workshop-migrate-$REL and must stay under 63 chars.
                      # The prefix is 24 characters, so $REL has 39 to spend. It used to have more:
                      # the pre-rebrand prefix was six characters shorter, so a release tag that fit
                      # before can now push the Job name past the limit and the apply is rejected.
...
docker build -f backend/Dockerfile                 -t $REGISTRY/design-workshop-api:$REL .
docker build -f backend/Dockerfile --target prisma -t $REGISTRY/design-workshop-migrate:$REL .
docker push $REGISTRY/design-workshop-api:$REL && docker push $REGISTRY/design-workshop-migrate:$REL
...
kubectl -n $NS wait --for=condition=complete --timeout=10m job/design-workshop-migrate-$REL \
  || { kubectl -n $NS logs job/design-workshop-migrate-$REL --tail=100; exit 1; }

# 4. Watch the rollout. progressDeadlineSeconds is 600, so this returns either way.
kubectl -n $NS rollout status deployment/design-workshop-api --timeout=10m
kubectl -n $NS rollout status deployment/design-workshop-queue --timeout=10m
...
kubectl -n $NS run smoke --rm -it --restart=Never --image=curlimages/curl -- \
  curl -fsS http://design-workshop-api/health/ready
```

**Rollback** is `kubectl -n $NS rollout undo deployment/design-workshop-api`. Note it rolls back *code*,
...
```bash
kubectl -n $NS get externalsecret design-workshop-secrets   # STATUS should be SecretSynced
kubectl -n $NS describe externalsecret design-workshop-secrets
```
...
- `kubeconform -strict -kubernetes-version 1.31.0`, with the datreeio CRD catalog supplying the
  ...
  **Not re-run after the Design Prototype Workshop rename** — the render still produces the same
  12 resources per overlay, and only `metadata.name`/label *values* changed, but the schema pass
  itself is from before. Re-run it before the next cluster apply.
- The `replacements` were checked in the rendered output: `design-workshop/dev|staging|prod/*` on all
  eleven secret keys, and Job names `design-workshop-migrate-dev` / `-staging` / `-REPLACE_RELEASE`.

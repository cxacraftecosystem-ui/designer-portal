...
This document describes one router, so it has one source of truth: `DATASETS` and the route
declarations in `backend/app/api/routes/datasets.py`, plus the CSV column derivation in
`backend/app/services/record_fields.py`. Every claim above is a property of those, and every one is
checkable in under a minute:

| Claim | How to check |
|---|---|
| The dataset catalogue | `GET /api/datasets` with a `dataset:read` token returns the live catalogue. Diff it against §3. Adding a dataset is one entry in `DATASETS`, so this is the row most likely to go stale |
| The token's containment to this router | `backend/tests/test_dataset_api.py` asserts that a `dataset:read` token is refused everywhere else. If that test is deleted, this claim becomes unverified |
| Identity masking behaviour | `backend/tests/test_identity_masking.py`, and `_identity_viewer` in `backend/app/api/routes/datasets.py` |
| Streaming and keyset paging | Read the generator functions in the route module — they must never grow an `OFFSET` |
| The error table | The `HTTPException` raises in the same module |

Endpoint and test **counts** are not written here; they are generated into
[REPO_FACTS.md](REPO_FACTS.md).

**Review triggers:** an entry added to or removed from `DATASETS`; a change to the token minting or
scope check in `backend/app/api/routes/datasets.py`; a change to which roles may see artisan identity
numbers; a change to `STREAM_BATCH` or to the paging strategy; any new query parameter on the paged
route.

**Known unverified:** the throughput and memory figures implied by "constant memory" have not been
measured against a production-sized table on the live box — they are a property of the generator
shape, which is inspectable, not of a benchmark anyone has run. That `X-Dataset-Total` matches the
row count a client eventually receives is asserted by the code but is not covered by an end-to-end
test against a table being written concurrently.

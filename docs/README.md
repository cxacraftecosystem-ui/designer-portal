...
| [PERMISSIONS.md](PERMISSIONS.md) | The **eleven**-tier role ladder (**eight** until 2026-09-13, when the three directorate tiers landed), the full capability matrix, the review state machine, the late-submission gate, and the **six** access systems layered on top — including the design-workshop viewer grant, the questionnaire visibility that follows it, the read-only inspector scope the `INSPECTOR` tier reaches a workshop through (§4.5, added 2026-08-27; this row said “four” until that day), and the oversight scope an Assistant or Regional Director monitors a workshop through (§4.6, added 2026-09-13; this row said “five” until 2026-09-16, and §4.6's own heading had called itself the sixth all along). Also the three rulings of 2026-09-15 — who may be OFFERED as a workshop's designer and why that set is narrower than the set who may write in one, who may appoint an inspector, and why the officer's designer picker had to be a fifth door rather than a widened gate |
...
| [AUDIT-2026-08-30.md](AUDIT-2026-08-30.md) | A dated snapshot taken against a MOVING tree: the 2026-08-30 scalability, ergonomics, concurrency and offline audit, ranked by severity from data loss down. Unlike [AUDIT-2026-08-15.md](AUDIT-2026-08-15.md) its line pins are re-pinned rather than frozen, because they were made into files that five lanes were rewriting as it was drafted — see its own "How this document is kept true". §7's fourteen-row asymmetry table is the most actionable part: a fix on either client is a specification for the other |
...
| [DECISION-questionnaire-voice-note-consent.md](DECISION-questionnaire-voice-note-consent.md) | What governs sending a questionnaire voice note to a transcription provider, now that `/questionnaire` posts one for an immediate transcript. **Built:** the interview's own design workshop or no request at all — no second consent regime, and the picker stays optional. The half that had to change on the server is the queue, which read a questionnaire clip as "not workshop material" and sent it ungated: a clip could be refused at the microphone and handed to ElevenLabs by the drain two hours later. Also carries the recording ceiling, and why "Not edited" is a claim only the interview form may make |
| [DECISION-design-workshop-data-in-view-data.md](DECISION-design-workshop-data-in-view-data.md) | Who may read the 22-stage record through View Data and Search, and who may take it out — a SPLIT rather than one gate: professors view, admins and the master admin also download. Also the seven implementation choices behind it: why 44 entity tables become 16 tabs plus an index page that names what did not fit, why design workshops are a fourth taxonomy root instead of a branch under the legacy one, where a photograph's stage comes from when no column carries it, why the map keeps five search buckets while search gets six, what a design-workshop text query does NOT look inside and what indexing it would cost, and why `design-workshop-dataset.zip` was renamed rather than left asserting something false |
| [DECISION-ministry-surfaces-web-only.md](DECISION-ministry-surfaces-web-only.md) | The owner's 2026-09-15 ruling that ministry-facing and administrative features are web + backend + database only, with deliberately no Android parity — a CARVE-OUT from this repository's normal rule, argued rather than asserted because five separate mechanisms here enforce the opposite. The split is not "ministry data versus designer data" but AUTHORING versus consuming: the annual plan, the sanction register, oversight and the ministry desk are typed in at a desk, and everything they produce — the workshop, the designer's account, the participant roster — is read on a phone in a courtyard. Names what is **not** covered: the record forms still require both clients, and the release that shipped these four web-only screens shipped the tool form to all four |
| [DECISION-ministry-orange-action-controls.md](DECISION-ministry-orange-action-controls.md) | The owner's 2026-09-20 reversal of one clause of the record above: the ministry's screens wear the ministry's colour on their ACTION controls too, not only on grounds and borders. Records what was overruled (both copies kept verbatim — the CSS block's own header and the test name `the ministry accent is spent on no action control`), what was **not** (the global `:focus-visible` outline, because `--purple-700` also draws two app-wide marks; and `--bg-0`/`--card`, because `applyPreferences` rewrites the mobile address bar on a PREFERENCE change and never on a navigation, so an orange canvas could never be corrected), and how the two MEASUREMENTS the old ruling rested on are answered rather than waived — the unscopeable `hover:shadow-cta` token by a `cta-ministry` rung at hue 45, and the ΔE 0.004 collision with `amber-800` by role, the secondary button keeping `ink-900` so the one control where the two inks would have met does not have them meet |
| [DECISION-mega-cards-and-group-colour.md](DECISION-mega-cards-and-group-colour.md) | The owner's 2026-09-20 ruling that the dashboard's four headed sections become MEGA CARDS — collapsed on a first visit, colour-coded, two in a row on a large screen and one on a phone, on web and on Android alike, with the same treatment on `/questionnaire`. Records the colour BOUNDARY that made it compatible with "purple-700 is the only action colour": a tone buys an `aria-hidden` icon chip, a hover border and a chevron, and never a button, an input, a focus ring, a card ground or a left-edge rule — which `globals.css` is already a tombstone for. Records the four new ramps (`archive`, `errand`, `steward`, `mango`), why none may be named after a stock Tailwind scale (`amber` in this repo is the standing proof that a colliding key DEEP-MERGES), and the measurement behind the mango: the named site's `--primary: 39 80% 40%` is `oklch(0.637 0.129 75.1)`, which `mango-500` reproduces — while the hue-45 `ministry` ramp STAYS, because `#FFA600` on white is 1.96:1 and cannot be an action colour. Also records the one stated exception to collapsed-by-default (`/questionnaire`'s capture card, because `?new=1` and `questionnaire-capture.spec.ts` both land on it), what the collapse cost `feature-entry-points.spec.ts` and how that spec was made stronger rather than weaker, and the eight measured ways the "My questionnaires" card was the wrong size |

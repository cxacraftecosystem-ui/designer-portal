...
| **The app's own install path against this endpoint** | **STILL NOT VERIFIED — and the reason has changed, so read the new one rather than the remembered one. Re-read on 2026-08-20 by opening the app.** The blocker this row recorded is GONE: it said "the client half needs a per-file artifact shape before any app surface can fetch from here", and that shape is now `android/app/src/main/java/com/designprototype/workshop/data/DwAsrModelEndpoint.kt` — the path builder, the manifest reading and `DwAsrEndpointState` — consumed by `DwAsrModelController.downloadFromEndpoint` in `DwAsrModelInstallUi.kt` under `DwAsrModelSource.DEPLOYMENT_ENDPOINT`. It is a route BESIDE `DW_ASR_MODEL_ARTIFACTS` rather than a row inside it, which is why `DwAsrModelArtifact`'s single-`https://`-**container** `require` did not have to be widened; that catalogue still pins exactly one row, the GitHub `.tar.bz2` at `DwAsrContainerFormat.TAR_BZ2`, whose format flag is `false`. **What is unverified is now the LOOP, not the shape**: nothing has driven that client against a real instance of this endpoint. `DwAsrModelEndpointTest` is a desktop-JVM test of the pure halves; an instrumented probe would need `androidTest`, which cannot be compiled in the environment this was written in (no Gradle network, and Docker down for the server side). **Do not flip this row to verified on the strength of the client existing.** The honest state is *client written, not yet driven against the endpoint*, and the fleet cannot drive it either until an operator sets `ASR_MODEL_DIR` (§3) — until then every handset is told nothing is published and takes the `NOT_PUBLISHED` arm. The handset proof in §4.1 is still `nc` + `sha256sum` on the phone, **not `DwDownload.kt` and not `downloadFromEndpoint`** |
...
| That the entitlement is the workshop set and not a rank | Parametrised over every role on the ladder, with PROFESSOR **and** `INSPECTOR` asserted **refused** — both outrank a designer and both sit outside `can_run_design_workshops`. **That row was FALSE when it was written and was corrected on 2026-08-27**: the two parametrisations were hand-kept tuples naming seven of the eight tiers, and `INSPECTOR` (rank 37, added the same day) was in neither, so nothing asserted an inspector's answer at all — the exact hole this row told a reader to check for and then reported as closed. They are now DERIVED in `test_asr_model_download.py` from `deps.DESIGN_WORKSHOP_ROLES` and `deps.ROLE_RANK` (`_MAY_DOWNLOAD` / `_MAY_NOT_DOWNLOAD`), which is a partition of the ladder by construction, so a ninth tier joins one side or the other with no edit here. Nothing in `test_role_ladder_parity.py` would have caught the tuples — its sweep stops at `frontend/` and `android/`, as its own docstring says. The same derivation absorbs the three directorate tiers added 2026-09-13 with no edit here, which is what "a partition of the ladder by construction" was for |

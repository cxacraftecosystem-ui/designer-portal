...
_PLACES: tuple[Place, ...] = (
    ...
    # Odisha — Sambalpuri bandha (ikat), the western handloom belt.
    #
    # ADDED BECAUSE THE MAP WAS DRAWING A LIE OF OMISSION. This table held thirteen places across
    # seven states — the clusters the baseline corpus happened to name — and Odisha was not one of
    # them. Every artisan of a Barpali workshop therefore failed to resolve, and `_artisan_points`
    # folded all twenty of them onto the state capital: a report whose map said "the artisans came
    # from Bhubaneswar" about a cluster 300 km away. The figure rendered, which is why it was easy
    # to miss; it was simply wrong.
    #
    # Coordinates are the published location of the named town or district headquarters, checked by
    # eye, exactly as the rows above. The atlas remains a CURATED table and not a gazetteer — see
    # the note at the head of `_PLACES` — so a cluster in a state not listed here still folds to its
    # capital. `scripts/build_boundaries.py` already joins 752 of India's 795 districts from a
    # published source, and emitting a district-centroid table from that join is the proper
    # nationwide fix; it needs a `--fetch` against the source dataset.
    Place("barpali", "Barpali", "Bargarh district, Odisha", "Odisha",
          21.1918, 83.5906, Precision.TOWN, ("barpali",), district="Bargarh"),
    Place("bargarh", "Bargarh", "Bargarh district, Odisha", "Odisha",
          21.3335, 83.6190, Precision.TOWN, ("bargarh", "baragarh"), district="Bargarh"),
    Place("bhatli", "Bhatli", "Bargarh district, Odisha", "Odisha",
          21.2833, 83.4500, Precision.TOWN, ("bhatli",), district="Bargarh"),
    Place("sohela", "Sohela", "Bargarh district, Odisha", "Odisha",
          21.2333, 83.4667, Precision.TOWN, ("sohela",), district="Bargarh"),
    Place("remunda", "Remunda", "Bargarh district, Odisha", "Odisha",
          21.2167, 83.6833, Precision.TOWN, ("remunda",), district="Bargarh"),
    Place("sambalpur", "Sambalpur", "Sambalpur district, Odisha", "Odisha",
          21.4669, 83.9812, Precision.TOWN, ("sambalpur", "sambhalpur"), district="Sambalpur"),
    Place("sonepur", "Sonepur", "Subarnapur district, Odisha", "Odisha",
          20.8333, 83.9167, Precision.TOWN, ("sonepur", "subarnapur", "sonpur"), district="Subarnapur"),
    Place("boudh", "Boudh", "Boudh district, Odisha", "Odisha",
          20.8379, 84.3264, Precision.TOWN, ("boudh", "baudh"), district="Boudh"),
    Place("nuapatna", "Nuapatna", "Cuttack district, Odisha", "Odisha",
          20.4167, 85.9333, Precision.TOWN, ("nuapatna", "maniabandha"), district="Cuttack"),
    Place("pipili", "Pipili", "Puri district, Odisha", "Odisha",
          20.1132, 85.8318, Precision.TOWN, ("pipili", "pipli"), district="Puri"),
    Place("raghurajpur", "Raghurajpur", "Puri district, Odisha (Pattachitra)", "Odisha",
          19.8833, 85.8000, Precision.TOWN, ("raghurajpur",), district="Puri"),
    Place("kotpad", "Kotpad", "Koraput district, Odisha", "Odisha",
          19.1500, 82.3167, Precision.TOWN, ("kotpad",), district="Koraput"),
    # Uttarakhand — Ringal

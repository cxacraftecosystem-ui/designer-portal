...
1. The separate `fieldrepo-queue` service writing transcripts and measurements onto media rows.
   That is the live systemd unit's real name, not a line the "Design Prototype Workshop" rebrand
   missed — the reasoning is in the module docstring of `backend/app/worker.py`.
...
docker exec design-workshop-redis redis-cli --scan --pattern 'fr:*'
```

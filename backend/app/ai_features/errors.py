...
class InvalidBrief(AiFeatureError):
    """The market-research brief is not something we are willing to send to a provider.

    Refused before the provider is loaded, because a blank query to a shopping API returns whatever
    the vendor feels like returning and spends a credit doing it.
    """

    code = "invalid_brief"


class ProvenanceViolation(AiFeatureError):
    """A RETRIEVED price reached a computation that may only see prices somebody was asked for.

    A programming error, deliberately loud. This is the one failure in the package that is about
    the integrity of a report rather than about a feature not running: a scraped listing counted as
    a respondent's answer would be indistinguishable, downstream, from fieldwork that never
    happened. It fails the call rather than filtering quietly, because a caller that mixed the two
    lists has a bug that will otherwise ship.
    """

    code = "provenance_violation"

    def __init__(self, message: str, *, offending: tuple[str, ...] = (), **kwargs: Any) -> None:
        super().__init__(message, **kwargs)
        self.offending = offending

    def as_dict(self) -> dict[str, Any]:
        payload = super().as_dict()
        payload["offending"] = list(self.offending)
        return payload

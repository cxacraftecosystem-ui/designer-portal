...
def _mounted_routes(router):
    """Every route reachable from ``router``, whichever way its FastAPI version stores them.

    ``include_router`` used to flatten a sub-router's routes into the parent's ``.routes``, and
    both tests below walked that list. FastAPI 0.141 keeps an ``_IncludedRouter`` wrapper there
    instead and resolves it when the app is built, so the flat walk finds nothing and the two
    tests fail with ``StopIteration`` — while the route itself answers 200, which is the tell
    that the assertion, not the application, is what broke. Recursing through the wrapper keeps
    the tests asserting what they were written to assert on either version.
    """
    for route in getattr(router, "routes", []):
        ...
        if inner is not None:
            ...
            for nested in _mounted_routes(inner):
                # The wrapper holds the sub-router's own paths; the prefix lives on the include.
                yield SimpleNamespace(
                    path=prefix + nested.path,
                    dependant=getattr(nested, "dependant", None),
                )


def test_the_census_is_mounted_at_the_public_path_the_cdn_rule_would_be_scoped_to() -> None:
    paths = {route.path for route in _mounted_routes(api_router)}
...
def test_the_census_asks_for_no_token(corpus) -> None:
    ...
    corpus()
    route = next(
        r for r in _mounted_routes(api_router) if r.path == "/api/public/census"
    )
